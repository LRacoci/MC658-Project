...
class Schedule {
  public:
	...
	vector< int > packCost(vector< pair<int, int > > & actorSet){
		...
		// sort scenesOrder ascending
		sort(scenesOrder.begin(), scenesOrder.end());
		...
		// Constroi empacotamento
		for (int i = 0; i < actorSet.size(); i++){
			// Para cada ator da ordem fornecida
			int a = actorSet[i].second;
			// Conta Cenas percorridas
			int scenesVisited = 0;
			for(int j = 0; j < scenesOrder.size(); j++){
				int sc = scenesOrder[j].second;
				if(packSet[sc]){
	...
	void bound() {
		...
		// Indica o primeiro dia de cada ator
		vector<int> first(nActors, 0);
		// Indica o ultimo dia de cada ator
		vector<int> last(nActors, nScenes-1);
		// Indica numero de dias conhecidos por ator
...
ostream& operator<<(ostream& os, const Schedule& s) {
	...
	*/
	...
	// Indicam os locais de insercao de novas escolhas
	int firstUnknown = (s.depth + 1) / 2, lastUnknown = nScenes - 1 - s.depth / 2;
	// Indica o primeiro dia de cada ator
	vector<int> first(nActors, 0);
	// Indica o ultimo dia de cada ator
	vector<int> last(nActors, nScenes - 1);
	...
	for (int i = 0; i < nActors; i++){
		...
		{
			nKnownDays[i]++;
			if (s.newScene < first[i])
			{
				first[i] = s.newScene;
			}
			if (s.newScene > last[i])
			{
				last[i] = s.newScene;
			}
		}
	...
	#else
	// Indicam os locais de insercao de novas escolhas
	int firstUnknown = s.firstUnknown, lastUnknown = s.lastUnknown;
	// Indica o primeiro dia de cada ator
	vector<int> first(s.first);
	// Indica o ultimo dia de cada ator
	vector<int> last(s.last);
	// Indica numero de dias conhecidos por ator
	vector<int> nKnownDays(s.nKnownDays);
	...
	os << "firstUnknown \t " << firstUnknown << endl;
	os << "lastUnknown \t " << lastUnknown << endl;
